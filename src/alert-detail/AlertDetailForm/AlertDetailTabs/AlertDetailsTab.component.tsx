...
const DEFAULT_EDR_FIELDS = ['isQuarantined', 'host', 'parentProcess', 'process']
...
const AlertDetailsTab = ({ alert, onFieldChange }: AlertDetailsTabProps) => {
  ...
  const [fields, setFields] = useState(
    isSIEMAlert(alert) ? DEFAULT_SIEM_FIELDS : DEFAULT_EDR_FIELDS
  )
  ...
  const handleBlur = <T extends SIEMAlert | EDRAlert>(
    field: keyof T,
    value: T[keyof T]
  ) => {
      ...
      <Grid container columnSpacing={2} rowSpacing={4}>
        <Grid size={12} display="flex" justifyContent="space-between">
          ...
          >
            <MenuList dense>
              {isSIEMAlert(alert)
                ? DEFAULT_SIEM_FIELDS.map((field) => (
                    <MenuItem
                      key={field}
                      onClick={() => handleMenuItemClick(field)}
                      disableRipple
                    >
                      <ListItemIcon>
                      ...
                      <Typography>
                        {FIELD_LABELS[field as keyof SIEMAlert]}
                      </Typography>
                    </MenuItem>
                  ))
                : DEFAULT_EDR_FIELDS.map((field) => (
                    ...
                    >
                      ...
                      <Typography>
                        {FIELD_LABELS[field as keyof EDRAlert]}
                      </Typography>
                    </MenuItem>
                  ))}
            </MenuList>
            ...
            return (
              <Grid size={FIELD_WIDTHS[siemField]}>
                <TextField
                  ...
                  fullWidth
                  onBlur={(evt) =>
                    handleBlur<SIEMAlert>(siemField, evt.target.value)
                  }
                />
              </Grid>
            )
              ...
              return (
                <Grid size={FIELD_WIDTHS.isQuarantined}>
                  ...
                  <Checkbox
                    ...
                    onBlur={(evt) =>
                      handleBlur<EDRAlert>('isQuarantined', evt.target.value)
                    }
                  />
                </Grid>
              )
            ...
            return (
              <Grid size={FIELD_WIDTHS[edrField]}>
                <TextField
                  ...
                  fullWidth
                  onBlur={(evt) =>
                    handleBlur<EDRAlert>(edrField, evt.target.value)
                  }
                />
              </Grid>
            )
